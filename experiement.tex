...
\gls{HEU} from natural uranium, with a tail assay close to $0.3w\%$. The design
...
operation, enriching natural uranium to about $3.5w\%$, with a tail assay close
...
\begin{figure}[ht] % replace 't' with 'b' to force it to be on the bottom
    ...
    red. The dashed orange line represent the alternative tail flow when
    tail recycling is considered.}
...
level are rounded up, to the first level is attibuted the remaining available
cascades.
...
required to reach the equilibrium assays value in the tail reprocessing cases.
