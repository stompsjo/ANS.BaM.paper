...
By construction, the very low uncertainty (Figure \ref{fig:power_full}) on the generated power is expected. The
transition scenario has been design to allow all the reactors to receive the
fuel as needed allowing them to be producing power all the time.
 \begin{figure}[ht] % replace 't' with 'b' to force it to be on the bottom
   \centering
   \includegraphics[scale=0.37]{power_full}
   \caption{Electric power generated as a function of time. The black line
   represents the reference calculation(wihtout uncertainty), and the blue zone
   represents the 1 $\sigma$ distribution uncertainty induced by the
   uncertainty of all the operational parameters.}\label{fig:power_full}
 \end{figure}
 %%  \begin{figure}[ht] % replace 't' with 'b' to force it to be on the bottom
 %%    \centering
