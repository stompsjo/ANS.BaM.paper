...
corresponds to the standard deviation of the output metrics from 199
...
updated\cite{mouginot_2018, cyclass} to evaluate the uncertainty of several
...
of the Nuclear Fuel Cycle Evaluation and Screening Report\cite{ES}. Pictured in Figure \ref{fig:cycle}, this is a
...
\begin{figure}[ht] % replace 't' with 'b' to force it to be on the bottom
    ...
    \caption{Electric power generated over time, with the full production,
        the \gls{LWR} contribution, and the \gls{FBR} contribution labeled in blue, orange, and green, respectively.}
    \label{fig:power}
\end{figure}
...
on demand.  Buffer storages (not shown in Figure \ref{fig:cycle}) are present
...
The main interests of nuclear archaeology involve highly enriched uranium production and
fissile inventories, so this study mainly focuses on the uncertainties of
the total quantity of natural uranium used and the total unused fissile
inventory (unused reprocessed plutonium).  
Generated power is also briefly being investigated to ensure the overall
uncertainty is not contaminated with a missed fuel loading, which could
be interesting but is not the focus of this work.

For this study, the 199 simulations were performed six times, and a reference calculation was also
computed. With the five parameters in Table
\ref{tab:package_uncertainty} (separation efficiency, residence time, \gls{UOX} fuel
enrichment, reactor cycle length, reactor thermal power) being tracked, the six sets are divided as follows: five sets where a single parameter is varied and 
the four others remain fixed, and one with all five parameters being varied. 
The reference calculation is a single
simulation with all five parameters fixed at the mean value.

In the following, each total uncertainty is reported at $\pm1\sigma$ around the
mean values of the corresponding uncertainty set at each time step.  The 
time-dependant relative uncertainties are computed as the standard deviation of a
