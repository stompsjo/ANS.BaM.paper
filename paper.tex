...
\bottomrule \end{tabular} \caption{Cascade effective $\theta$ as a function of
the cascade level and the case considered.} \label{tab:cascade_theta}
\end{table} As illustrated in Table \ref{tab:cascade_theta}, the effective cut
...
\subsubsection{Case 1}
As shows on Figure \ref{ }, without tails recycling, the number of cascade levels
required to enrich uranium up to $90\%$ is 5 for the
first case considered. Recycling the tails allows for a reduction in the number of
enrichment levels down to only 3.

This bump in enrichment can easily be explained, as the cascade does not have
the same number of stages, less work is spent on stripping as opposed to
enriching. Even compared to an ideal case, where the stripping
work of a stage is the exact inverse of the enriching work, a cascade with
less stripping stages will produce tails with a higher
enrichment than the feed of the previous level of

%Did you commit in the middle of a sentence or did you forget to finish?
