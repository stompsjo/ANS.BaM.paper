...
%% I'm not a fan of the use of the word ``errors'' here.  We should be careful
%% since (in English) this can meam ``mistakes''.  Uncertainty is a better
%% word, and then we'll have to distinguish between input uncertainties and
...
``operational'' uncertainty (from the expected behavior of the facilities
being simulated).
%% is this operational uncertainty really just a different kind of data?
...
uncertain.  Any uncertainty in fissile production quantities poses a material
accounting challenge, possibly
creating an opportunity for undetected diversion.  This study seeks
...
%% should there be a reference for Total Monte Carlo?
...
\midrule
Cycamore & Separation & Separation efficiency     \\ %% add hline in 2nd two columns
...
\end{figure}
%% Update this to match the one in the CVT poster?
...
%% is it really a constant rate then?  Maybe we need a better word to describe this
on demand.  Buffer storage facilities (not shown in Figure \ref{fig:cycle}) are present
%% You can't pluralize ``storage'' in English - it's considered uncountable
...
$10\%$.
%% Did you have to experiment to find the burnup uncertainty that results in a 10% enrichment uncertainty?
...
%% unused implies it's never used, but I don't think that's right.  You just mean the amount
%% that is being stored in a separated state any point in time, right?
...
be interesting but is not the focus of this work.
%% what do you mean by contaminated???  This probably deserves more explanation
...
uranium consumption.
%% Why does the relative uncertainty due to cyle time not approach 0?
...
%% I don't see what you mean by decrease up to 10%?
...
uncertainty is the cycle length.  As with the uranium needs, lower cycle length
implies more fuel loads.  Never the less, around year 65 (see Figure
%% Aren't you varying the cycle length equally to be smaller and larger, so why
%% focus on ``lower cycle lengths''?
\ref{fig:used_fuel}), the stock of used fuel available for reprocessing becomes
small enough to start limiting producton of fissile material.
