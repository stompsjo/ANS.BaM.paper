...
\midrule
320    & $320$       & $1.8$  & $0.105$  & $1^{3}$ & $13$       & 2     & 1.0  \\
\bottomrule
\end{tabular}
...
than $1^{-8}$.  As the cut remain fixed at each stage the flows do not need to
be recomputed.
...
(equation \eqref{eq:theta}). As the cascade remains ideal, the product and tails
...
are recomputed using equation \eqref{eq:flow}. The new flows are also
constrained by the fixed design of the cascade, number of centrifuges limiting
the flow at each level.
